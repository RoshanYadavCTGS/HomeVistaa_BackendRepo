...
exports.sendInquiryConfirmation = sendInquiryConfirmation;
exports.sendListingSubmissionEmail = sendListingSubmissionEmail;
...
const FROM = `"${env_1.env.SMTP_FROM_NAME}" <${env_1.env.SMTP_FROM_EMAIL}>`;
async function sendMail(to, subject, html) {
    // Always log and return without attempting SMTP transmission while commented out
    if (!transporter || !env_1.env.SMTP_ENABLED) {
        logger_1.logger.info(`[Email disabled / SMTP Commented Out] Would have sent to ${to}: ${subject}`);
        return;
    }
    /*
...
async function sendWelcomeEmail(name, email) {
    await sendMail(email, `Welcome to HomeVistaa, ${name}!`, `
...
async function sendPasswordResetEmail(email, resetUrl) {
    await sendMail(email, 'Reset Your HomeVistaa Password', `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto;">
...
async function sendInquiryConfirmation(name, email, propertyName) {
    await sendMail(email, 'Your Inquiry Has Been Received — HomeVistaa', `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto;">
...
async function sendListingSubmissionEmail(name, email, title) {
    await sendMail(email, 'Property Listing Submitted — HomeVistaa', `
    <div style="font-family: sans-serif; max-width: 600px; margin: auto;">
...
async function sendOtpEmail(email, otp) {
    ...
    logger_1.logger.info(`[OTP Sent] 6-digit authentication code for ${email} is: ${otp}`);
    const html = `
  <!DOCTYPE html>
  <html lang="en">
  <head>
